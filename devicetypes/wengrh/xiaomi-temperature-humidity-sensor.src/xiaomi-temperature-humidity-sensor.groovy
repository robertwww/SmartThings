    ...
    tiles(scale: 2) {
		multiAttributeTile(name:"temperature", type:"generic", width:6, height:4) {
            ...
            tileAttribute("device.humidity", key: "SECONDARY_CONTROL"){
                attributeState("humidity", label:'${currentValue}%', unit:"%", icon:"st.Weather.weather12", defaultState: true)
            }
            */
...
/*
		...
		valueTile("humidity2", "device.humidity") {
...
*/
        ...
        details(["temperature", "humidity", "pressure", "battery", "refresh"])
	}
}
...
private String parseValue(String description) {
    def linkText = getLinkText(device)
	if (description?.startsWith("temperature: ")) {
    	log.debug "Temperature zigbee.parseHAT:"
        log.debug zigbee.parseHATemperatureValue(description, "temperature: ", "F")
